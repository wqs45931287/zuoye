...
sub configure_module_bsd {
  ...
  if ($module eq 'vmmemctl') {
    ...
    db_add_answer('VMBLOCK_CONFED_FUSE', $moduleConfed eq 'yes' ? 'no' : 'yes');
  }
  return $moduleConfed;
}
...
sub write_vmware_config {
  ...
  if (vmware_product() eq 'ws') {
    ...
    $config->set('product.version', '10.1.6');
  }
  $config->set('product.buildNumber', '5214329');
